...
export default function AllProductsPage() {
  ...
  const handleToggleWishlist = async (e, product) => {
    e.preventDefault(); // Link navigation আটকানোর জন্য
    ...
    try {
      if (wishlistMap[pid]) {
        ...
        setWishlistMap((prev) => ({ ...prev, [pid]: res?._id || pid }));
      }
    } catch (e) {
      console.error(e);
    } finally {
    ...
    <div className="min-h-screen bg-surface py-10 md:py-16 px-4 sm:px-6 lg:px-12">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3 bg-surface-container-lowest border border-outline-variant/30 p-4 rounded-2xl mb-8">
          <div className="relative flex items-center col-span-1 md:col-span-2">
            ...
            <input
              ...
              className="w-full h-10 pl-10 pr-4 rounded-xl border border-outline-variant/40 bg-surface text-on-surface text-sm font-sans focus:outline-none focus:border-primary transition"
            />
          </div>
          ...
          <div className="relative flex items-center">
            ...
            >
              ...
              {["Pristine", "Like New", "Refurbished", "Good", "Used"].map(
                (c) => (
                  <option key={c} value={c}>
                    {c}
                  </option>
        ...
        {loading ? (
          ...
          <p className="text-on-surface-variant text-center py-16 font-sans">
              ...
              {paginatedProducts.map((product) => (
                <Link key={product._id} href={`/products/${product._id}`}>
                  <motion.article
                    ...
                    className="group bg-surface-container-lowest rounded-2xl overflow-hidden border border-outline-variant/30 flex flex-col h-full cursor-pointer hover:border-primary/30 transition-colors relative"
                  >
                    {/* Wishlist Button (❤️) */}
                    <button
                      onClick={(e) => handleToggleWishlist(e, product)}
                      disabled={wishlistLoadingId === product._id}
                      className="absolute top-3 right-3 z-10 p-2 bg-surface/80 backdrop-blur-md rounded-full border border-outline-variant/30 text-primary hover:scale-110 transition shadow-sm disabled:opacity-50 flex items-center justify-center min-w-[32px] min-h-[32px]"
                    >
                    ...
                    <div className="p-4 flex flex-col grow justify-between">
                      ...
                      <div className="flex justify-between items-center mt-3">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          />
                          <span className="text-xs text-on-surface-variant font-sans truncate max-w-20">
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-8 bg-surface-container-lowest border border-outline-variant/30 p-4 rounded-2xl">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="h-9 w-9 flex items-center justify-center rounded-xl border border-outline-variant/40 bg-surface text-on-surface-variant hover:bg-surface-container-high disabled:opacity-30 disabled:cursor-not-allowed transition text-sm font-bold"
                  >
                    ‹
                  </button>
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1)
                    ...
                    .map((item, idx) =>
                      item === "..." ? (
                        <span
                          key={`e-${idx}`}
                          className="h-9 w-9 flex items-center justify-center text-outline text-xs"
                        >
                          ···
                        </span>
                      ) : (
                        <button
                          ...
                          className={`h-9 w-9 flex items-center justify-center rounded-xl border text-xs font-bold transition-all ${currentPage === item ? "bg-primary border-primary text-on-primary shadow-sm" : "border-outline-variant/40 bg-surface text-on-surface-variant hover:bg-surface-container-high"}`}
                        >
                          {item}
                        </button>
                      ),
